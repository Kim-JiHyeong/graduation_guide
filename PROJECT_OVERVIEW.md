...
- Gemini 호출 실패 시 키 권한 오류, 모델명 오류, 사용량 제한, 모델 혼잡, 네트워크 차단을 구분해 사용자에게 안내한다.
...
요청은 `{ question, statusSummary }`, 성공 응답은 `{ answer }` 형식이다. 입력 누락은 400, 키 누락은 500으로 반환한다. Gemini 호출 실패는 상태 코드에 따라 API 키 권한 오류(401/403), 모델명 오류(404), 사용량 제한(429), 모델 혼잡(503), 네트워크 차단(EACCES) 메시지로 구분해 반환한다.
...
- 저장 구조 변경에 대비해 로딩 시 기본값을 보정하고, 보정된 데이터는 다시 `localStorage`에 저장한다.
- 명시적인 버전별 마이그레이션 체계는 없다.
...
- 사회봉사1·2, 해외봉사1·2는 카탈로그 과목으로 등록되어 있어 직접 입력 없이 `공통교양-인성` 영역에서 선택할 수 있다.
