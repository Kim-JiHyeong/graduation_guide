...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey) {
    return NextResponse.json(
      {
        error:
          "GEMINI_API_KEY가 설정되지 않았습니다. frontend/.env.local에 GEMINI_API_KEY=... 를 추가하고 서버를 재시작하세요.",
      },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    for (const modelName of models) {
      try {
        ...
        lastError = error;
        if (getApiStatus(error) !== 503) throw error;
      }
    }

    if (!response) throw lastError;

    const answer = response.text?.trim() || "답변을 생성하지 못했습니다.";
